...
import React, { useState, useCallback, FormEvent, ChangeEvent, useRef, useEffect } from 'react';
import { Search, Filter, AlertTriangle, CheckCircle, Clock, Map, X, ThumbsUp, ThumbsDown, Plus, Upload } from 'lucide-react';
...
}> = ({ info, onVerify, isPending, onVote }) => (
...
const IncidentReportForm: React.FC<{
  onSubmit: (incident: { description: string; category: string; image: File | null; lat: number; lng: number }) => void;
  onCancel: () => void;
  lat: number;
  lng: number;
}> = ({ onSubmit, onCancel, lat, lng }) => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (description && category) {
      onSubmit({ description, category, image, lat, lng });
    } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        <input
          ...
          style={{ display: 'none' }}
        />
        <button type="button" onClick={() => fileInputRef.current?.click()} className="px-2 py-1 bg-gray-200 rounded">
...
const DisasterInfoVerificationUI: React.FC = () => {
  ...
  const handleVerifyInfo = useCallback((id: number) => {
    setPendingInfo(prev => {
      ...
      if (infoToVerify) {
        setVerifiedInfo(prevVerified => {
          if (!prevVerified.some(item => item.id === id)) {
            return [...prevVerified, infoToVerify];
          }
          return prevVerified;
        });
      }
  ...
  const handleAddIncident = useCallback((lat: number, lng: number) => {
    setShowReportForm(true);
    setTempIncidentLocation({ lat, lng });
  }, []);

  const handleSubmitIncident = useCallback(async (newIncident: { description: string; category: string; image: File | null; lat: number; lng: number }) => {
    let imageUrl = '';
    if (newIncident.image && typeof window !== 'undefined') {
      imageUrl = URL.createObjectURL(newIncident.image);
    }
  ...
  const handleAddNews = useCallback((e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (newNewsText.trim()) {
      const newNews: InfoItem = {
        id: Date.now(),
        text: newNewsText.trim(),
        upvotes: 0,
        downvotes: 0
      };
      setPendingInfo(prevInfo => [newNews, ...prevInfo]);
      setNewNewsText('');
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
      <header className="mb-6 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <Image src="/alberta-logo.png" alt="Alberta Logo" width={50} height={50} className="h-10 w-10 rounded-full" />
          <Image src="/ualberta-logo.png" alt="University of Alberta Logo" width={50} height={50} className="h-10 w-10 rounded-full" />
          ...
          {showAddNewsForm && (
            <form onSubmit={handleAddNews} className="mb-4">
              <input
                type="text"
                value={newNewsText}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setNewNewsText(e.target.value)}
